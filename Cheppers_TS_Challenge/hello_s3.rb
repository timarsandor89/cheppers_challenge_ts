...
# Creating file for Hello world
File.open(user_file_name, 'w+') {|f| f.write('Hello, World!')}

# Creating bucket for files
new_bucket = s3.bucket(user_bucket_name)
new_bucket.create

# Creating objects and uploading the corresponding files
new_file_to_save = File.basename(user_file_name)
s3.bucket(user_bucket_name).object(new_file_to_save).upload_file(user_file_name)
